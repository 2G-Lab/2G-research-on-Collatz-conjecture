// ---------------------------------------- Collatz Coefficients
// C(n_i) = n_(i-1) / 2                                          : n_(i-1) even
// C(n_i) = ( n_(i-1) * CollatzFactor + CollatzTerm) / 2         : n_(i-1) odd
const CollatzFactor = 3n;
const CollatzTerm   = 7n;
// ---------------------------------------- Collatz Coefficients
...
function createChild (branchParity, parent_ID) {
  ...
  if (parentElement.type === SIMPLE) {
    ...
    if (parentElement.parity === ODD) {
      sequenceElement.N_i.factor = parentElement.N_i.factor * CollatzFactor;
      sequenceElement.N_i.term   = parentElement.N_i.term * CollatzFactor + CollatzTerm;
      // ---------------------------------------------------- v1.1
    ...
    if (sequenceElement.N_i.term === sequenceElement.N_0.term) { // LOOP !!!
      ...
      loopFound = sequenceElement.N_i.term === 0n || sequenceElement.N_i.term === CollatzTerm ? false : true;
